...
password_value=$(cat ./buzz-secrets.json | jq -r '.BUZZ_IM_ADMIN_DB_PASSWORD')
...
password_value=$(cat ./buzz-secrets.json | jq -r '.APNS_CERT')
set_buzz_password $source $target $password_key $password_value

source="NO"
target=./buzz-im-admin/buzzim-admin-secret.yaml
password_key=GCM_API_KEY_HERE
password_value=$(cat ./buzz-secrets.json | jq -r '.GCM_API_KEY')
set_buzz_password $source $target $password_key $password_value

source=./template/stolon-postgresql/secret.yaml
target=./stolon-postgresql/secret.yaml
password_key=SECRET_PASSWORD
password_value=$(cat ./buzz-secrets.json | jq -r '.STOLON_SECRET_PASSWORD')
set_buzz_password $source $target $password_key $password_value
...
password_value=$(cat ./buzz-secrets.json | jq -r '.STKEEPER_PG_REPL_PASSWORD')
set_buzz_password $source $target $password_key $password_value
